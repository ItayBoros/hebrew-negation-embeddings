...
$\times$ selection $\in$ \{cv, train\}) and report, per model, whichever
configuration reaches the highest pairwise accuracy among those satisfying
the same $\mathrm{sim\_unrelated} \leq 0.5$ constraint.
...
\texttt{oriel9p/AlephBERT-FT-HebNLI-LCHAIM}. \textbf{Why not the released checkpoint.}
...
\multirow{3}{*}{multilingual-e5}
 ...
 & projection (classifier, $\gamma$=12, relaxed) & 0.97 & sim\_unrel 0.79 (over budget) \\
...
\multirow{3}{*}{sambert}
 ...
 & projection (classifier, $\gamma$=20) & 0.92 & sim\_unrel 0.39 \\
 & nli\_rerank ($\lambda$=0.35) & \textbf{0.99} & STS 0.67 \\
\bottomrule
\end{tabular}
\caption{Best available \texttt{projection} configuration per model among
the 8 (direction $\times$ centering $\times$ selection) combinations
satisfying $\mathrm{sim\_unrelated} \leq 0.5$ (multilingual-e5: none
satisfy it, so the lowest-\emph{sim\_unrelated} point is shown and flagged
relaxed). $\mathrm{sim\_unrelated}$ and STS are on different scales and not
directly comparable, so both trade-off guards are reported alongside rather
than merged; \texttt{nli\_rerank} wins on every model on accuracy, with a
real semantic-similarity guard rather than a proxy, except that its own
guard did not hold at test time for LaBSE (see Table \ref{tab:nlirerank}).
\texttt{projection} needs no second model at inference time and, using the
best safe configuration per model rather than one fixed one, closes most of
the accuracy gap on three of four models.}
